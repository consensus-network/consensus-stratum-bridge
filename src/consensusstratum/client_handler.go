...
func (c *clientListener) NewBlockAvailable(kapi *ConsensusApi) {
	c.clientLock.Lock()
		...
		go func(client *gostratum.StratumContext) {
			...
			if err != nil {
				if strings.Contains(err.Error(), "Could not decode address") {
					...
					client.Logger.Error(fmt.Sprintf("failed fetching new block template from consensus, malformed address: %s", err))
					client.Disconnect() // unrecoverable
				} else {
					RecordWorkerError(client.WalletAddr, ErrFailedBlockFetch)
					client.Logger.Error(fmt.Sprintf("failed fetching new block template from consensus: %s", err))
				}
				return
			}
			...
			if !state.initialized {
				...
				state.stratumDiff = newConsensusDiff()
	...
	if time.Since(c.lastBalanceCheck) > balanceDelay {
		...
		if len(addresses) > 0 {
			go func() {
				balances, err := kapi.consensusd.GetBalancesByAddresses(addresses)
				if err != nil {
					c.logger.Warn("failed to get balances from consensus, prom stats will be out of date", zap.Error(err))
					return
				}
